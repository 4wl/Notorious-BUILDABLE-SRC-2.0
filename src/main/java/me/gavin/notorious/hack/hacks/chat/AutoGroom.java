...
public class AutoGroom extends Hack
{
    ...
    public void onUpdate(final PlayerLivingUpdateEvent event) {
        ...
        final List<EntityPlayer> playerEntities = (List<EntityPlayer>)AutoGroom.mc.world.playerEntities;
        for (final Entity e : playerEntities) {
            if (e.getName().equals(AutoGroom.mc.player.getName())) {
                continue;
            }
            ...
            for (final String name : this.peopleNearbyNew) {
                if (!this.peopleInArea.contains(name)) {
                    if (this.timer.hasTicksPassed((long)(this.delay.getValue() * 20.0f))) {
                        AutoGroom.mc.player.sendChatMessage("/msg " + name + " hewwo wittle kitten, come be my wittle discord girl? | discord.gg/BU9g9HgGBa");
                        this.timer.reset();
                    }
        ...
        if (this.peopleInArea.size() > 0) {
            for (final String name : this.peopleInArea) {
                if (!this.peopleNearbyNew.contains(name)) {
                    ...
                    AutoGroom.mc.player.sendChatMessage("/msg " + name + " ow no pwease dont weave me my wittle kitten UWU");
                    this.timer.reset();
                }
            }
