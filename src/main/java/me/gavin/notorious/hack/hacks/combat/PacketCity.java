...
public class PacketCity extends Hack
{
    ...
    public void onUpdate(final PlayerLivingUpdateEvent event) {
        ...
        if (this.firstRun && this.mineTarget != null) {
            this.firstRun = false;
            this.notorious.messageManager.sendMessage(TextFormatting.WHITE + " Attempting to mine: " + ChatFormatting.BLUE + this.closestTarget.getName());
        }
        this.findCityBlock();
        if (this.mineTarget != null) {
            int newSlot = -1;
            for (int i = 0; i < 9; ++i) {
                final ItemStack stack = PacketCity.mc.player.inventory.getStackInSlot(i);
            ...
            if (newSlot != -1) {
                PacketCity.mc.player.inventory.currentItem = newSlot;
            }
            ...
            PacketCity.mc.player.swingArm(EnumHand.MAIN_HAND);
            PacketCity.mc.player.connection.sendPacket((Packet)new CPacketPlayerDigging(CPacketPlayerDigging.Action.START_DESTROY_BLOCK, this.mineTarget, EnumFacing.DOWN));
            PacketCity.mc.player.connection.sendPacket((Packet)new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, this.mineTarget, EnumFacing.DOWN));
            if (!wasEnabled) {
                this.notorious.hackManager.getHack(PacketMine.class).toggle();
            }
        }
    ...
    public BlockPos findCityBlock() {
        ...
        if (PacketCity.mc.player.getPositionVector().distanceTo(vec) <= dist) {
            ...
            if ((!this.canBreak(targetX) && !this.canBreak(targetXMinus) && !this.canBreak(targetZ) && !this.canBreak(targetZMinus)) || PacketCity.mc.player.getPositionVector().distanceTo(vec) > dist) {
                this.mineTarget = null;
            }
    ...
    private boolean canBreak(final BlockPos pos) {
        final IBlockState blockState = PacketCity.mc.world.getBlockState(pos);
        final Block block = blockState.getBlock();
        return block.getBlockHardness(blockState, (World)PacketCity.mc.world, pos) != -1.0f;
    }
    
    private void findClosestTarget() {
        final List<EntityPlayer> playerList = (List<EntityPlayer>)PacketCity.mc.world.playerEntities;
        this.closestTarget = null;
        for (final EntityPlayer target : playerList) {
            if (target == PacketCity.mc.player) {
                continue;
            }
            ...
            if (this.closestTarget == null) {
                ...
                if (PacketCity.mc.player.getDistance((Entity)target) >= PacketCity.mc.player.getDistance((Entity)this.closestTarget)) {
                    continue;
                }
                this.closestTarget = target;
            }
