...
@RegisterHack(name = "Chams", description = "highlight entities", category = Hack.Category.Render)
public class Chams extends Hack
{
    ...
    private void setupChams() {
        ...
        Chams.mc.getRenderManager().setRenderShadow(false);
    ...
    private void endChams() {
        Chams.mc.getRenderManager().setRenderShadow(Chams.mc.gameSettings.entityShadows);
        if (this.walls.getValue()) {
            this.endDepthRange();
        }
