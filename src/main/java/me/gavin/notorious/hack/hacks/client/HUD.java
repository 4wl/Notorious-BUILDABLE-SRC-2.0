...
public class HUD extends Hack
{
    ...
    public void onRenderWatermark(final RenderGameOverlayEvent.Text event) {
        if (this.waterMark.isEnabled()) {
            ...
            final ServerData data = HUD.mc.getCurrentServerData();
            String string = "";
            ...
            if (this.name.isEnabled()) {
                string = string + (Notorious.INSTANCE.hackManager.getHack(Font.class).isEnabled() ? " | " : " \u23d0 ") + HUD.mc.player.getDisplayNameString();
            }
            if (this.ping.isEnabled()) {
                string = string + (Notorious.INSTANCE.hackManager.getHack(Font.class).isEnabled() ? " | " : " \u23d0 ") + "Ping:" + this.getPing((EntityPlayer)HUD.mc.player);
            }
            if (this.ip.isEnabled()) {
                if (HUD.mc.getConnection() != null && HUD.mc.getCurrentServerData() != null && HUD.mc.getCurrentServerData().serverIP != null) {
                    string = string + (Notorious.INSTANCE.hackManager.getHack(Font.class).isEnabled() ? " | " : " \u23d0 ") + data.serverIP;
                }
                else {
            ...
            if (this.fps.isEnabled()) {
                string = string + (Notorious.INSTANCE.hackManager.getHack(Font.class).isEnabled() ? " | " : " \u23d0 ") + "FPS:" + Minecraft.getDebugFPS();
            }
            Color colorColor;
            ...
            if (this.renderMode.getMode().equals("Basic")) {
                if (font.isEnabled()) {
                    ...
                    HUD.mc.fontRenderer.drawStringWithShadow(string, (float)((int)this.xWatermark.getValue() + 2), (float)(int)this.yWatermark.getValue(), intColor);
                }
            }
            if (this.renderMode.getMode().equals("Skeet")) {
                Gui.drawRect((int)this.xWatermark.getValue() - 1, (int)this.yWatermark.getValue() - 1, (int)this.xWatermark.getValue() + (Notorious.INSTANCE.hackManager.getHack(Font.class).isEnabled() ? this.notorious.fontRenderer.getStringWidth(string) : HUD.mc.fontRenderer.getStringWidth(string)) + 5, (int)this.yWatermark.getValue() + HUD.mc.fontRenderer.FONT_HEIGHT + 4, new Color(80, 80, 80, 255).getRGB());
                Gui.drawRect((int)this.xWatermark.getValue(), (int)this.yWatermark.getValue(), (int)this.xWatermark.getValue() + (Notorious.INSTANCE.hackManager.getHack(Font.class).isEnabled() ? this.notorious.fontRenderer.getStringWidth(string) : HUD.mc.fontRenderer.getStringWidth(string)) + 4, (int)this.yWatermark.getValue() + HUD.mc.fontRenderer.FONT_HEIGHT + 3, new Color(0, 0, 0, 255).getRGB());
                Gui.drawRect((int)this.xWatermark.getValue(), (int)this.yWatermark.getValue(), (int)this.xWatermark.getValue() + (Notorious.INSTANCE.hackManager.getHack(Font.class).isEnabled() ? this.notorious.fontRenderer.getStringWidth(string) : HUD.mc.fontRenderer.getStringWidth(string)) + 4, (int)this.yWatermark.getValue() + 1, ColorUtil.getRainbow(this.length.getValue(), this.saturation.getValue()));
                if (font.isEnabled()) {
                    ...
                    HUD.mc.fontRenderer.drawStringWithShadow(string, (float)((int)this.xWatermark.getValue() + 2), (float)((int)this.yWatermark.getValue() + 3), intColor);
                }
            }
        }
            ...
            for (final Hack hack : this.notorious.hackManager.getSortedHacks()) {
                if (hack.isEnabled() || (!hack.isEnabled() && System.currentTimeMillis() - hack.lastDisabledTime < 50L)) {
                    ...
                    if (font2.isEnabled()) {
                        ...
                        startPos = HUD.mc.fontRenderer.getStringWidth(name) + 2;
                    }
                    int color;
                    ...
                    double x;
                    if (hack.isEnabled()) {
                        x = -startPos + (startPos + 1.0) * MathHelper.clamp((double)AnimationUtil.getSmooth2Animation(250.0f, (float)(System.currentTimeMillis() - hack.lastEnabledTime)), 0.0, 1.0);
                    }
                    else {
                        x = startPos * -MathHelper.clamp((double)AnimationUtil.getSmooth2Animation(250.0f, (float)(System.currentTimeMillis() - hack.lastDisabledTime)), 0.0, 1.0);
                    }
                    double y = yOffset;
                    ...
                    if (font2.isEnabled()) {
                        if (this.renderMode.getMode().equals("Skeet")) {
                            Gui.drawRect((int)x - 3, (int)y - 2, (int)(x + startPos + 1.0), (int)(y + this.notorious.fontRenderer.getHeight() + 2.0), -1879048192);
                            Gui.drawRect((int)x - 4, (int)y - 2, (int)x + 1, (int)(y + this.notorious.fontRenderer.getHeight() + 2.0), color);
                        }
                    }
                    else if (this.renderMode.getMode().equals("Skeet")) {
                        Gui.drawRect((int)x - 3, (int)y - 2, (int)(x + startPos + 1.0), (int)(y + HUD.mc.fontRenderer.FONT_HEIGHT + 2.0), -1879048192);
                        Gui.drawRect((int)x - 4, (int)y - 2, (int)x + 1, (int)(y + HUD.mc.fontRenderer.FONT_HEIGHT + 2.0), color);
                    }
                    if (font2.isEnabled()) {
                        this.notorious.fontRenderer.drawStringWithShadow(name, x + 2.0, y, new Color(color));
                    }
                    else {
                        HUD.mc.fontRenderer.drawStringWithShadow(name, (float)((int)x + 2), (float)(int)y, color);
                    }
                    if (font2.isEnabled()) {
                        yOffset += this.notorious.fontRenderer.getHeight() + (int)this.wordSpacing.getValue();
                    }
                    else {
                        yOffset += HUD.mc.fontRenderer.FONT_HEIGHT + (int)this.wordSpacing.getValue();
                    }
                }
            }
        }
        if (this.friendList.isEnabled()) {
            ...
            HUD.mc.fontRenderer.drawStringWithShadow(homies, this.xFriendList.getValue(), this.yFriendList.getValue(), color2);
            if (!this.notorious.friend.getFriends().isEmpty()) {
                for (final Friend f : this.notorious.friend.getFriends()) {
                    HUD.mc.fontRenderer.drawStringWithShadow(f.getName(), this.xFriendList.getValue(), this.yFriendList.getValue() + yOffset + 6.0f, -1);
                    yOffset += (int)(HUD.mc.fontRenderer.FONT_HEIGHT + 0.5);
                }
            }
        }
        if (this.welcomer.isEnabled()) {
            ...
            if (font3.isEnabled()) {
                this.notorious.fontRenderer.drawStringWithShadow(this.welcomerMode.getMode().equals("Normal") ? (this.WelcomeMessages() + HUD.mc.player.getDisplayNameString()) : ("Sub to " + HUD.mc.player.getDisplayNameString() + " on YouTube"), this.xWelcomer.getValue(), this.yWelcomer.getValue(), new Color(color3));
            }
            else {
                HUD.mc.fontRenderer.drawStringWithShadow(this.welcomerMode.getMode().equals("Normal") ? (this.WelcomeMessages() + HUD.mc.player.getDisplayNameString()) : ("Sub to " + HUD.mc.player.getDisplayNameString() + " on YouTube"), this.xWelcomer.getValue(), this.yWelcomer.getValue(), color3);
            }
        }
    }
    ...
    public int getPing(final EntityPlayer player) {
        int ping = 0;
        try {
            ping = MathUtil.clamp(Objects.requireNonNull(HUD.mc.getConnection()).getPlayerInfo(player.getUniqueID()).getResponseTime(), 1, 99999);
        }
