...
public class AutoLog extends Hack
{
    ...
    public void onUpdate(final PlayerLivingUpdateEvent event) {
        if (AutoLog.mc.player.getHealth() <= this.health.getValue()) {
            Objects.requireNonNull(AutoLog.mc.getConnection()).handleDisconnect(new SPacketDisconnect((ITextComponent)new TextComponentString("Logged at [" + AutoLog.mc.player.getHealth() + "]")));
            this.toggle();
        }
    }
}
