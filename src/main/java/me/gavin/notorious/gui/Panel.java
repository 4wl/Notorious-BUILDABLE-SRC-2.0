...
public class Panel extends AbstractDragComponent
{
    ...
    public void render(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        Gui.drawRect(this.x, this.y, this.x + this.width, this.y + this.height - 2, color);
        if (font.isEnabled()) {
            Notorious.INSTANCE.fontRenderer.drawStringWithShadow(this.category.name(), this.x + 3.0f, this.y + 5.0f, Color.WHITE);
        }
        else {
            Minecraft.getMinecraft().fontRenderer.drawStringWithShadow(this.category.name(), this.x + 3.0f, this.y + 3.0f, new Color(255, 255, 255).getRGB());
        }
