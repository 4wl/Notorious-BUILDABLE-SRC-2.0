...
public class CFontRenderer extends CFont
{
    ...
    public float drawString(final String text, double x, double y, final Color gsColor, final boolean shadow) {
        --x;
        y -= 2.0;
        Color color = gsColor;
        if (text == null) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (shouldContinue) {
                ...
                if (equals) {
                    ...
                    this.drawString(escapeString, x + currentWidth, y, Color.WHITE, shadow);
                    break;
                }
                this.drawString(String.valueOf(currentChar).equals("§") ? "" : String.valueOf(currentChar), x + currentWidth, y, shouldRainbow ? currentColor : Color.WHITE, shadow);
